  ...
  return (
    <FullWidthLayout sidebar={<Sidebar links={posts} />}>
      <div className="space-y-8">
        <div>
          <h1 className="text-4xl font-bold mb-4 bg-gradient-to-r from-green-600 to-emerald-600 text-transparent bg-clip-text">
            Our Facilities
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {flatPosts.map((post) => (
            <Link 
              key={post.slug}
              to={post.href}
              className="group relative overflow-hidden rounded-lg border p-5 hover:border-green-600 transition-colors bg-white hover:shadow-lg"
            >
